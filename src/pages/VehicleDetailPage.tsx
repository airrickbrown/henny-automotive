...
// ── Spec cell used in the desktop spec grid ─────────────────────────────────
function SpecCell({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="font-label text-[10px] uppercase tracking-widest text-on-surface-variant mb-1 font-bold">
        {label}
      </p>
      <p className="font-headline text-xl font-bold italic text-white uppercase leading-tight">
        {value}
      </p>
    </div>
  )
...
export default function VehicleDetailPage() {
    ...
    return (
      <PageWrapper>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center px-6">
          <span className="font-material text-6xl text-on-surface-variant/30">
            directions_car
          </span>
          <h1 className="font-headline text-3xl font-black uppercase text-white">
            Vehicle Not Found
          </h1>
  ...
  const metaDescription = vehicle.overview.length > 155
    ? vehicle.overview.slice(0, 152) + '...'
    : vehicle.overview
  ...
  return (
    <PageWrapper>
      ...
      <section className="hidden md:grid grid-cols-12 gap-4 max-w-[1440px] mx-auto px-8 h-[640px] mb-12">
        {/* Main image — 8 cols with title overlay */}
        <div className="col-span-8 h-full overflow-hidden relative group">
          {vehicle.images[0] ? (
            ...
            <div className="w-full h-full bg-surface-container flex items-center justify-center">
              <span className="font-material text-8xl text-on-surface-variant/20">
                directions_car
              </span>
            </div>
          )}
            ...
            <div key={i} className="overflow-hidden bg-surface-container">
              {vehicle.images[i] ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <span className="font-material text-4xl text-on-surface-variant/20">
                    directions_car
                  </span>
                </div>
              )}
      ...
      {/* ── Mobile Gallery (horizontal scroll snap) ──────────────── */}
      <section className="md:hidden relative mb-0">
        ...
        <div className="absolute bottom-3 right-4 bg-black/50 backdrop-blur-sm px-3 py-1">
          <span className="font-label text-xs tracking-widest text-white">
            1 / {vehicle.images.length || 1}
          </span>
        </div>
      </section>

      {/* ── Mobile: Vehicle identity + primary WA CTA ────────────── */}
      <div className="md:hidden px-6 py-8 bg-surface">
        <div className="flex justify-between items-start mb-2">
          {vehicle.status && (
            <span className="font-label text-xs uppercase tracking-[0.2em] text-primary-container font-bold">
              {vehicle.status}
            </span>
          )}
        ...
        <div className="mt-5 flex items-baseline gap-3">
          <span
            className={`font-headline text-3xl font-black ${isPriceOnReq ? 'text-primary-container' : 'text-white'}`}
          >
            {displayPrice}
          </span>
        ...
        <div className="flex flex-col md:grid md:grid-cols-12 gap-10 md:gap-12">
          ...
          <div className="md:col-span-7 space-y-8">
            ...
            <div className="bg-surface-container-low p-8 md:p-10 space-y-8">
              ...
              {/* Badges — desktop only (mobile shows above) */}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6 border-b border-outline-variant/10 pb-8">
                ...
                {vehicle.spec.power && (
                  <SpecCell label="Power" value={vehicle.spec.power} />
                )}
                {vehicle.spec.zeroToSixty && (
                  <SpecCell label="0–60 MPH" value={vehicle.spec.zeroToSixty} />
                )}
                {vehicle.spec.drivetrain && (
                  <SpecCell label="Drivetrain" value={vehicle.spec.drivetrain} />
                )}
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {vehicle.keyFeatures.map((f) => (
                      <span
                        key={f}
                        className="px-4 py-2 border border-outline-variant/20 font-label text-[10px] uppercase tracking-widest text-white"
                      >
                        {f}
                      </span>
                    ))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-surface-container-high p-6 md:p-8 flex items-start gap-4">
                <span className="font-material text-3xl text-primary-container flex-shrink-0">
                  public
                </span>
                <div>
              ...
              <div className="bg-surface-container-high p-6 md:p-8 flex items-start gap-4">
                <span className="font-material text-3xl text-secondary flex-shrink-0">
                  location_on
                </span>
                <div>
                  <p className="font-label text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-1">
            ...
            {/* Mobile: Snapchat + Call row */}
            <div className="md:hidden grid grid-cols-2 gap-4">
              <a
                href={SNAPCHAT_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 py-4 bg-surface-container-high active:scale-95 transition-transform"
              >
                <span className="font-material text-xl text-[#FFFC00]">swipe</span>
                <span className="font-headline font-bold uppercase text-[10px] tracking-widest text-white">
                  Snapchat
                </span>
              </a>
              <a
          ...
          {/* ── Action card column (col-5, sticky) ─────────────── */}
          <div className="hidden md:block md:col-span-5">
            <div className="bg-surface-container-low p-8 sticky top-28 space-y-5">
              ...
              <div className="mb-6">
                ...
                <p
                  className={`font-headline text-5xl font-black italic ${isPriceOnReq ? 'text-primary-container' : 'text-white'}`}
                >
                  {displayPrice}
                </p>
              </div>
              ...
              <a
                href="tel:+233000000000"
                className="w-full border-2 border-outline-variant/30 hover:border-white text-white font-headline font-black text-base py-5 flex items-center justify-center gap-3 transition-colors duration-150"
              >
            ...
            <div className="hidden md:grid grid-cols-3 gap-8">
              {related.map((v) => (
                ...
                >
                  ...
                  {v.status && (
                    <p className="font-label text-[10px] font-bold text-primary-container tracking-widest uppercase mb-1">
                      {v.status}
                    </p>
                  )}
                  <h3 className="font-headline text-xl font-bold text-white mb-2 italic uppercase">
                    {v.year} {v.make} {v.model}
                  </h3>
                  <div className="flex gap-3 text-on-surface-variant font-label text-xs uppercase tracking-widest">
                    <span>{v.spec.mileage}</span>
                    <span>•</span>
                  ...
                  <div className="bg-surface-container-low overflow-hidden">
                    <div className="aspect-video overflow-hidden">
                      {v.images[0] ? (
                        ...
                        <div className="w-full h-full bg-surface-container flex items-center justify-center">
                          <span className="font-material text-3xl text-on-surface-variant/20">
                            directions_car
                          </span>
                        </div>
                      )}
                    ...
                    <div className="p-4">
                      <h4 className="font-headline font-bold text-white uppercase italic text-lg leading-tight">
                        {v.year} {v.make} {v.model}
                      </h4>
